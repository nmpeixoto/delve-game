...
## Overview
This skill instructs the agent to run the DELVE automated test bot (`autoplay_test.js`), evaluate its performance and causes of death from decision traces, and inject smarter heuristics into `bot_brain.js`. The goal is to maximize honest average floor depth and win rate without exploiting runner artifacts.
...
- Examine `outcomes`, floor statistics, `totalTurnsPlayed`, console errors, and the printed decision traces from `autoplay_test.js`.
...
- Add or update focused regression tests before changing strategy when feasible:
  - Use `bot_brain_test.js` for decision-logic rules.
  - Use `combat_test.js` for engine/combat invariants.
...
- Run `node autoplay_test.js 5` or a larger requested batch to measure behavior.
- Run `node test.js` when UI/runtime behavior or `dungeon.html` changes.
